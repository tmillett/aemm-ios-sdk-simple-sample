...
- (NSURL *)manifestURL {
	return [[self.assetSource.assetService.baseURL URLByAppendingPathComponent:self.assetSource.identifier] URLByAppendingPathComponent:@".caas.json"];
}
...
- (NSString *)localPathForAsset:(NSDictionary *)asset {
	NSString *localPathFragment = asset[@"localPath"];
	if (!localPathFragment) {
		localPathFragment = [[NSURL URLWithString:asset[@"url"]] path];
	}
	return [self.assetSource.rootFilePath stringByAppendingPathComponent:localPathFragment];
}
...
- (void)downloadAssets {
	...
	if (assetsToDownload.count == 0) {
		...
		for (NSDictionary *asset in assetsToDownload) {
			...
			info.localPath = [self localPathForAsset:asset];
	...
	if (self.assetsRemoved.count) {
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
			...
			for (NSDictionary *asset in [self.assetsRemoved allValues]) {
				if (![[NSFileManager defaultManager] removeItemAtPath:[self localPathForAsset:asset] error:nil]) {
					ASSERT_FAIL(@"");
				}
			}
